...
router.get('/', [
  ...
  try {
    ...
    // Organizer-only access for listing games
    ...
    // Filter by organizer - only show games created by this organizer
    ...
    const result = await Game.findAll(filters, { page, limit });
...
router.get('/:id', [authenticateSupabaseToken], async (req, res) => {
  try {
    ...
    const hasAccess = event.organizer === req.user.id ||
                      event.participants?.some(p => p.user === req.user.id);
...
// @access  Private (Event organizer)
 router.post('/', [
  ...
  try {
    ...
    // Organizer-only access for creating games
    if (req.user.role !== 'organizer') {
      return res.status(403).json({
        success: false,
        message: 'Only event organizers can create games'
      });
    }
...
// @access  Private (Game creator)
 router.post('/:id/start', [authenticateSupabaseToken], async (req, res) => {
  try {
    ...
    const updatedGame = await Game.update(req.params.id, {
      status: 'active',
      startedAt: new Date(),
      updatedAt: new Date()
    });
...
// @access  Private (Game creator)
 router.get('/:id/analytics', [authenticateSupabaseToken], async (req, res) => {
  try {
    ...
    const analytics = await Submission.getGameAnalytics(req.params.id);
...
// @access  Private (Game participants or creator)
 router.get('/:id/leaderboard', [authenticateSupabaseToken], async (req, res) => {
  try {
    ...
    const leaderboard = await Submission.getGameLeaderboard(req.params.id);
